from flask import Flask,render_template,request,redirect,url_for,flash,session,jsonify, render_template, escape
...
def contact():
    ...
    data = contact(contact_id = userId, userName = userName, phone = phone, emailAddress = emailAddress, msg = msg)
    db.session.add(data)
    db.session.commit()
    ...
    # db1.execute("INSERT INTO contact (userName, phone, emailAddress, msg) VALUES (:userName, :phone, :emailAddress, :msg)",
    #         {"userName": userName, "phone": phone, "emailAddress" :emailAddress, "msg" :msg})
    #
    # db1.commit()
